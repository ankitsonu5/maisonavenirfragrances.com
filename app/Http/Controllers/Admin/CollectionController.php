<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\BaseController;
use App\Models\Admin\Collection;
use App\Traits\UpdatedTrait;
use Illuminate\Http\Request;

class CollectionController extends BaseController
{
    protected $model, $module;
    use UpdatedTrait;
    public function __construct(Collection $model)
    {
        parent::__construct($model, 'collection');
    }

    /**
     * Display a listing of the resource.
     */
    public function index(Request $request)
    {
        $query = $this->model::query();
        $data = $query->orderBy('order', )->paginate(50);
        return view('admin.' . $this->module . '.index', compact('data'));
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create(Collection $collection)
    {

        return view('admin.' . $this->module . '.create', ['row' => $collection]);
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $formInput = $request->all();
        $category = $this->model::create($formInput);
        return $this->redirectAfterSave($request->FormButton, $category->id);
    }

    /**
     * Display the specified resource.
     */
    public function show(Collection $collection)
    {
        return view('admin.' . $this->module . '.show', ['row' => $collection]);
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(Collection $collection)
    {

        return view('admin.' . $this->module . '.edit', ['row' => $collection]);
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, Collection $collection)
    {
        $formInput = $request->all();
        $collection->update($formInput);
        return $this->redirectAfterSave($request->FormButton, $collection->id);
    }


    /**
     * Remove the specified resource from storage.
     */
    public function destroy(Request $request, Collection $collection)
    {
        $collection->delete();
        $request->flash('success', 'Deleted successfully');
        return back();
    }
}
